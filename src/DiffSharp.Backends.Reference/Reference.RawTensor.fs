...
/// Optimised layout engine for no array-allocation metadata overhead.
[<Struct; IsReadOnly>]
type TensorLayout =
    val Shape: int[]
    val Strides: int[]
    ...
    new(shape: int[]) =
        let strides = Array.zeroCreate shape.Length
        let mutable acc = 1
        for i = shape.Length - 1 downto 0 do
            strides.[i] <- acc
            acc <- acc * shape.[i]
        { Shape = shape; Strides = strides; Offset = 0 }

    new(shape: int[], strides: int[], offset: int) =
        { Shape = shape; Strides = strides; Offset = offset }

    /// Calculates the flat array coordinate index for an input multi-dimensional index.
    member inline this.GetFlatIndex(indices: ReadOnlySpan<int>) =
        let mutable idx = this.Offset
        for i = 0 to indices.Length - 1 do
            idx <- idx + (indices.[i] * this.Strides.[i])
        idx

    /// Decodes a flat index back into pre-allocated multi-dimensional coordinates.
    /// Zero heap allocation.
    member inline this.GetMultiIndex(flatIndex: int, destination: Span<int>) =
        let mutable remainder = flatIndex - this.Offset
        for i = 0 to this.Shape.Length - 1 do
            destination.[i] <- remainder / this.Strides.[i]
            remainder <- remainder % this.Strides.[i]


/// This is the base class for all RawTensorXyz types.
/// All type-independent operations are implemented directly on this class. 
[<AbstractClass>]
type FastRawTensorCPU<'T when 'T: unmanaged and 'T: struct>(values: 'T[], shape: Shape, dtype: Dtype, device: Device) =
    inherit RawTensor()
    ...
    member _.Layout : TensorLayout = layout

    member internal t.IndexToFlatIndex(index:int[]) =
        t.Layout.GetFlatIndex(ReadOnlySpan index)
    
    member internal t.FlatIndexToIndex(flatIndex:int) =
        let result = Array.zeroCreate t.Dim
        t.Layout.GetMultiIndex(flatIndex, Span result)
        result
    ...
    member t.Item
        with get ([<System.ParamArray>] index:int[]) =
            ...
            t.Values.[t.Layout.GetFlatIndex(ReadOnlySpan index)]

        and set ([<System.ParamArray>] index:int[]) v =
            if index.Length <> t.Dim then failwithf "Expecting a %id index" t.Dim
            t.Values.[t.Layout.GetFlatIndex(ReadOnlySpan index)] <- v
    ...
    override t.GetSlice(fullBounds:int[,]) =
        ...
        let totalElements = shapeLength shape
        let array = Array.zeroCreate totalElements
        
        let dims = t.Dim
        let coordsBuffer = Array.zeroCreate dims
        let currentCoords = Span<int> coordsBuffer
        let mutable arrayi = 0
        let rec slice (dimIndex: int) =

            let startIdx = fullBounds.[dimIndex, 0]
            let endIdx = fullBounds.[dimIndex, 1]

            if dimIndex = dims - 1 then

                for i = startIdx to endIdx do
                    currentCoords.[dimIndex] <- i
                    let flatIdx = t.Layout.GetFlatIndex(ReadOnlySpan(coordsBuffer))
                    array.[arrayi] <- t.Values.[flatIdx]
                    arrayi <- arrayi + 1
        ...
        if totalElements > 0 then slice 0
        t.MakeLike(array, shape)

    override t.Clone() = t.MakeLike(Array.copy t.Values, t.Shape)

    abstract member MakeLike: values: 'T[] * layout: TensorLayout * ?device: Device -> RawTensor
    abstract member MakeLike: values: 'T[] * shape: Shape * ?device: Device -> RawTensor
    
    override x.ComputeHash() = hash shape + System.Collections.StructuralComparisons.StructuralEqualityComparer.GetHashCode x.Values
    
    override t.Expand (newShape: Shape) =
        if newShape.Length = 1 && newShape.[0] = 0
        then
            t.MakeLike([||], newShape)
        elif t.Shape = newShape then t :> _ 
        else
            Shape.checkCanExpand t.Shape newShape

            let n = shapeLength newShape
            let result = Array.zeroCreate n

            let trim = newShape.Length - t.Shape.Length
            ...
            if jshape.Length = 0 then 
                ...
                let scalarVal = t.Values.[0]
                Span<'T>(result).Fill scalarVal
            else
                let strideFlags = Array.zeroCreate jshape.Length
                let tStrides = t.Layout.Strides

                let shapeNextMultiplier = Array.zeroCreate jshape.Length
                let jshapeNextMultiplier = Array.zeroCreate jshape.Length

                for d = 0 to jshape.Length - 1 do
                    strideFlags.[d] <- if t.Shape.[d] = jshape.[d] then 1 else 0
                    if d < jshape.Length - 1 then
                        shapeNextMultiplier.[d] <- t.Shape.[d + 1]
                        jshapeNextMultiplier.[d] <- jshape.[d + 1]

                let rec loopFast ibase jbase d = 
                    let strideD = strideFlags.[d]
                    if d < jshape.Length - 1 then
                        let mutable iD = 0
                        let limit = jshape.[d] - 1
                        let sNext = shapeNextMultiplier.[d]
                        let jNext = jshapeNextMultiplier.[d]
                        
                        for jD = 0 to limit do 
                            let ibaseD = (ibase + iD) * sNext
                            let jbaseD = (jbase + jD) * jNext
                            loopFast ibaseD jbaseD (d + 1)
                            iD <- iD + strideD
                    else
                        // Leaf node hot loop: direct array assignments
                        let mutable iD = 0
                        let limit = jshape.[d] - 1
                        let vals = t.Values
                        for jD = 0 to limit do 
                            result.[jbase + jD] <- vals.[ibase + iD]
                            iD <- iD + strideD
                let startJBaseStride = if jshape.Length > 0 then jshape.[0] else 1
                for jP = 0 to exp - 1 do
                    loopFast 0 (jP * startJBaseStride) 0
    ...
    override t.ToValues() =
        ...
        let isContiguous = layout.Offset = 0 && layout.Strides.[layout.Strides.Length - 1] = 1

        match t.Dim with
        | 0 -> box values.[layout.Offset]
        | 1 ->
            let res = Array.zeroCreate shape.[0]
            if isContiguous then ReadOnlySpan<'T>(vals, 0, shape.[0]).CopyTo(Span<'T> res)
            else for i = 0 to shape.[0] - 1 do res.[i] <- t.[ [|i|] ]
            upcast res
...
//         t.MakeLike(result, outShape)
    ...
    override t.SplitT(sizes, dim) =
        ...
        if layout.Offset = 0 && layout.Strides.[layout.Strides.Length - 1] = 1 then
            let srcSpan = ReadOnlySpan<'T> values
            let mutable srcOffset = 0
            for j1 = 0 to m1 - 1 do 
                for k = 0 to n - 1 do
                    let d = sizes.[k]
                    let blockSize = d * m3
                    let destOffset = j1 * blockSize
                    
                    let targetDest = Span<'T>(results.[k], destOffset, blockSize)
                    let targetSrc = srcSpan.Slice(srcOffset, blockSize)
                    
                    targetSrc.CopyTo targetDest
                    srcOffset <- srcOffset + blockSize
        ()
...
//             t.MakeLike(rvalues, outShape))
    ...
    override t.PermuteT(permutation) =
        ...
        let newStrides = Array.zeroCreate t.Dim
        for i = 0 to t.Dim - 1 do
            let originalAxis = permutation.[i]
            newStrides.[i] <- t.Layout.Strides.[originalAxis]

        let permutedLayout = TensorLayout(newShape, newStrides, layout.Offset)
        t.MakeLike(t.Values, permutedLayout)

    override t.TransposeT(dim0, dim1) =
        let dims = t.Dim
        let permutation = Array.init dims id
...
//         upcast result
    ...
    override t.ViewT(shape:Shape) =
        ...
        if t.Layout.Offset = 0 && t.Layout.Strides.[t.Layout.Strides.Length - 1] = 1
        then
            t.MakeLike(t.Values, TensorLayout shape)
        else
            let len = t.Nelement
            let contiguousCopy = Array.zeroCreate<'T> len
            let srcSpan = t.AsSpan()
            let dstSpan = Span<'T>(contiguousCopy)
            srcSpan.CopyTo dstSpan
            t.MakeLike(contiguousCopy, TensorLayout shape)
    ...
    override t.Cast(dtype: Dtype) =
        if dtype = t.Dtype then 
            ...
            let inline convertArray<'Target when 'Target: unmanaged and 'Target: struct> (convLambda: 'T -> 'Target) =
                let dstVals = Array.zeroCreate<'Target> len
                let dstSpan = Span<'Target>(dstVals)
                for i = 0 to len - 1 do
                    dstSpan.[i] <- convLambda srcSpan.[i]
                t.MakeLike(dstVals, t.Shape)

            match dtype with
            | Dtype.Float32 -> 
                upcast convertArray<float32> (fun x -> Convert.ToSingle x)
...
//                 | _ -> tflat.ToValues()
    ...
    member t.SetValues(tmp: RawTensor) = checkMutable(); values <- (tmp :?> FastRawTensorCPU<'T>).Values
